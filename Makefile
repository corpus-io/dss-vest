all                   :; DAPP_BUILD_OPTIMIZE=1 DAPP_BUILD_OPTIMIZE_RUNS=200 dapp --use solc:0.8.17 build
clean                 :; dapp clean && rm -rf crytic-export corpus
test                  :; ./test.sh match="$(match)" block="$(block)" match-test="$(match-test)" match-contract="$(match-contract)"
solc-select           :; pip3 install solc-select && solc-select install 0.8.17
echidna-mintable      :; ./echidna/echidna.sh mintable
echidna-suckable      :; ./echidna/echidna.sh suckable
echidna-transferrable :; ./echidna/echidna.sh transferrable
certora-mintable      :; certoraRun --solc ~/.solc-select/artifacts/solc-0.8.17/solc-0.8.17 certora/DssVest4Certora.sol:DssVestMintable4Certora certora/DSToken.sol certora/MockAuthority.sol --link DssVestMintable4Certora:gem=DSToken DSToken:authority=MockAuthority --verify DssVestMintable4Certora:certora/DssVestMintable.spec --solc_args "['--optimize','--optimize-runs','200']" --rule_sanity $(if $(rule),--rule $(rule),) --multi_assert_check --short_output
certora-suckable      :; certoraRun --solc ~/.solc-select/artifacts/solc-0.8.17/solc-0.8.17 src/DssVest.sol:DssVestSuckable certora/ChainLog.sol certora/Vat.sol certora/DaiJoin.sol certora/Dai.sol --link DssVestSuckable:chainlog=ChainLog DssVestSuckable:vat=Vat DssVestSuckable:daiJoin=DaiJoin DaiJoin:vat=Vat DaiJoin:dai=Dai --verify DssVestSuckable:certora/DssVestSuckable.spec --solc_args "['--optimize','--optimize-runs','200']" --rule_sanity  $(if $(rule),--rule $(rule),) --multi_assert_check --short_output
certora-transferrable :; certoraRun --solc ~/.solc-select/artifacts/solc-0.8.17/solc-0.8.17 src/DssVest.sol:DssVestTransferrable certora/Dai.sol --link DssVestTransferrable:gem=Dai --verify DssVestTransferrable:certora/DssVestTransferrable.spec --solc_args "['--optimize','--optimize-runs','200']" --rule_sanity  $(if $(rule),--rule $(rule),) --multi_assert_check --short_output
deploy-mintable       :; make && dapp create DssVestMintable $(gem)
deploy-suckable       :; make && dapp create DssVestSuckable 0xdA0Ab1e0017DEbCd72Be8599041a2aa3bA7e740F
deploy-transferrable  :; make && dapp create DssVestTransferrable $(owner) $(gem)
